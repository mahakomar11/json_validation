...
def create_error(file_name, schema_name, error_message, is_valid=False, file_property='', schema_path=''):
    return {
        'File': file_name,
        'Schema': schema_name,
        'Error': error_message,
...
def validate_events_data(events_dict, schema_validators):
    ...
    for name, event in events_dict.items():
        ...
        if event_key not in schema_validators:
            errors_list.extend(compare_with_all_schemas(name, event, schema_validators))
        else:
            event_errors = validate_data(schema_validators[event_key], name, event)
            if len(event_errors) == 0:
                errors_list.append(create_error(name, event_key, 'File is OK', is_valid=True))
            else:
                errors_list.extend(event_errors)
    return errors_list


def compare_with_all_schemas(name, event, schema_validators):
    errors_list = []
...
if __name__ == '__main__':
    get_validation_table(sys.argv[1])
